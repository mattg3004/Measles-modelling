  ...
  for(j in 1:num.steps){
    ...
    total.births.vac = births.vac + total.births.vac
    #print(paste('births.vac should be =', total.births.vac + initial.vac_0))
    ...
    foi.ages1                    =      foi.by.next.gen ( mixing.matrix, disease.state, infectious.indices, time.step , infectious.period, beta, demographic.ages, num.comps)
    ...
    for ( i in 1:length(demographic.ages[,1]) ){
      ...
      foi.age                     =      min(1,foi.ages1[age+1, ])
      estimate.inf                =      foi.age*disease.state[((age*num.comps)+1)]  +  estimate.inf
      if(age == 0)
      {
        foi.by.time[j,1]      =      foi.age
      }
      change.matrix.by.age        =      stochastic.transmission.matrix.exposed.included  (age , disease.state , foi.age  , demographic.ages , time.step ,   rho, mu, num.comps)
   ...
   if(j %% 399  == 0){
     disease.state.slow.pre.infection = disease.state
   }
  }
  years   =   ceiling(t/365)
